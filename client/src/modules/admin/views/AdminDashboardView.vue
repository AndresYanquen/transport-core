...
const kpis = computed(() => [
  {
    label: "Servicios hoy",
    value: todaysRides.value.length,
    icon: Car,
    tone: "violet",
  ...
  },
  {
    ...
    value: onlineDrivers.value.length,
    icon: Users,
    tone: "amber",
    trend: "Disponibles ahora",
  },
  {
  ...
  },
  {
    ...
    icon: Users,
    tone: "blue",
    trend: "Con servicio activo",
  },
  {
    label: "Asignación promedio",
    value: fmtMs(metrics.value?.rides?.avgAssignmentMs),
    icon: Clock3,
    tone: "rose",
    trend: "Tiempo de despacho",
  },
  {
    label: "Cancelaciones del día",
    value: canceledToday.value,
    icon: AlertTriangle,
    tone: "orange",
    trend: "Seguimiento requerido",
  },
  {
    label: "Ingresos del día",
    value: fmtCurrency(revenueToday.value),
    icon: DollarSign,
    tone: "lime",
    trend: "Servicios finalizados",
  },
]);
...
const servicesByType = computed(() => {
  ...
  return Array.from(counts.entries()).map(([label, value]) => ({
    ...
    class: {
      Taxi: "bg-slate-950",
      Domicilio: "bg-amber-400",
      Baúl: "bg-blue-500",
      Despinchada: "bg-emerald-500",
    }[label] || "bg-slate-500",
  }));
});
...
<template>
  <section class="grid gap-4 bg-slate-50 p-4">
    ...
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <div
        v-for="kpi in kpis"
        :key="kpi.label"
        :class="['rounded-md border p-4 shadow-sm', toneClasses[kpi.tone]?.card]"
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <div :class="['mt-2 text-xs font-semibold', toneClasses[kpi.tone]?.trend]">
              {{ kpi.trend }}
            </div>
          </div>
          <div :class="['grid h-11 w-11 place-items-center rounded-md', toneClasses[kpi.tone]?.icon]">
    ...
    <div class="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
      <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div class="relative h-72 overflow-hidden rounded-md border border-slate-200 bg-slate-100">
          <div class="absolute inset-0 bg-[linear-gradient(to_right,rgba(148,163,184,0.24)_1px,transparent_1px),linear-gradient(to_bottom,rgba(148,163,184,0.24)_1px,transparent_1px)] bg-[size:32px_32px]"></div>
          <div class="absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(59,130,246,0.16),transparent_28%),radial-gradient(circle_at_68%_35%,rgba(16,185,129,0.16),transparent_24%),radial-gradient(circle_at_50%_75%,rgba(245,158,11,0.14),transparent_26%)]"></div>
          <div
      ...
      <div class="grid gap-4">
        <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div class="mt-3 grid gap-2">
            <div v-for="[label, value] in operationalStatus" :key="label" class="flex items-center justify-between rounded-md bg-slate-50 px-3 py-2 ring-1 ring-slate-100">
        ...
        <div class="rounded-md border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div class="mt-3 grid gap-3">
            <div v-for="service in servicesByType" :key="service.label">
              ...
              <div :class="['h-full rounded-full', service.class]" :style="{ width: service.width }"></div>
              </div>
            </div>
          </div>
        </div>
